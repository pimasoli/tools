...
def main():
	parser = argparse.ArgumentParser(description='Convert a single text-file or multiple text-files from source-encoding to target-encoding.')
	...
	parser.add_argument('-t', '--target-encoding', default='UTF-8', help='the target encoding (default=UTF-8)', dest='target_encoding', required=True)
	parser.add_argument('-r', '--recursive', default=False, help='recursively convert a directory instead of a single file', action='store_true', dest='recursive')
